...
angular.module('app', []).controller('ListIoctrlCSVCtrl', ['$scope', '$state', '$rootScope', 'fileUpload', 'IoctrlCSVFactory', function ($scope, $state, $rootScope, fileUpload, IoctrlCSVFactory) {

    // 宣告vm
    $scope.vm = {};

    // 動作
    $scope.vm.uploadIoctrlCSV = function () {
        if (confirm("上傳檔案視網路速度需一至數分鐘")) {
            $("#loading").fadeIn("fast");
            var file = $scope.myFile;
            console.log('file is ' + JSON.stringify(file));
            var uploadUrl = "../../mvc/IoctrlCSV";
    ...
    $scope.vm.getAllData = function () {
        IoctrlCSVFactory.getAll().success(function (data) {
    ...
    $scope.gridOptions.columnDefs = [
       ...
       { name: 'IO_BASE', width: 100 },
       { name: 'DELIVER', width: 100 },
       { name: 'STOCK', width: 100 },
