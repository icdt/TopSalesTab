...
angular.module('app', []).controller('ListForSaleCSVCtrl', ['$scope', '$state', '$rootScope', 'fileUpload', 'ForSaleCSVFactory', function ($scope, $state, $rootScope, fileUpload, ForSaleCSVFactory) {

    // 宣告vm
    $scope.vm = {};

    // 動作
    $scope.vm.uploadForSaleCSV = function () {
        if (confirm("上傳檔案視網路速度需一至數分鐘")) {
            $("#loading").fadeIn("fast");
            var file = $scope.myFile;
            console.log('file is ' + JSON.stringify(file));
            var uploadUrl = "../../mvc/ForSaleCSV";
    ...
    $scope.vm.getAllData = function () {
        ForSaleCSVFactory.getAll().success(function (data) {
    ...
    $scope.gridOptions.columnDefs = [
       ...
       { name: 'SALE_FDATE', width: 100 },
       { name: 'SALE_EDATE', width: 100 },
